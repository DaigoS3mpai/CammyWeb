...
const CategoryPage = () => {
    ...
    categoryName === "galeria"
      ? (() => {
          ...
          items.forEach((media) => {
            ...
            // Lógica de portada: preferimos imagen sobre video
  ...
  return (
    <motion.div
      className="flex-1 p-10 bg-cover bg-center bg-fixed text-white min-h-screen relative overflow-hidden"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      {decorSrc && (
        <img
          ...
          className="hidden md:block pointer-events-none select-none absolute right-56 top-5 w-64 drop-shadow-xl"
        />
      )}
