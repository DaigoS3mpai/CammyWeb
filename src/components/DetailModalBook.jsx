...
const DetailModalBook = ({ item, type, onClose }) => {
  ...
  // 🔹 NUEVO: Cargar galería asociada (imagenes y videos) para paginar
  useEffect(() => {
    if (!item?.id) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 🔹 NUEVO: subida de mediaFiles (imagenes en clases, img+video en proyectos)
  ...
  // 🧮 Paginación dinámica (página 1 = descripción; de la 2 en adelante = 5 items por página)
              ...
              <div className="w-1/2 p-8 bg-[#fefbf6] flex flex-col justify-between">
                {page === 1 ? (
                  ...
                  <>
                    ...
                    {/* NUEVO: input de subida unificado (clases: solo imagen | proyectos: imagen+video) */}
                    {editMode && (
                      <div className="mb-4">
